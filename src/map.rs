use std::{ 
    arch::x86_64::{
        ...
        _mm_cmpeq_epi8,
         ...
         _mm_set1_epi8, 
         _mm_setzero_si128 
    }, cell::Cell, hash::Hash, marker::PhantomData, mem::MaybeUninit, sync::atomic::AtomicU32 
};
...
pub struct HashMap<K, V> {
    // ------ HOT DATA --
    directory_ptr: * mut u32, // 8 Bytes, ROUTER, Boxed Slice Ptr + Len // make it atm ptr later
    ...
    K: PartialEq + Hash 
{
    ...
    pub fn new() -> anyhow::Result<Self> {
        let directory_arena_size = 128 * 1024 * 1024;
        let payload_arena_size = 1024 * 1024 * 1024;
        ...
        // MMAP Optimizations 
        unsafe {
            libc::madvise(payload_ptr as *mut libc::c_void, payload_arena_size, libc::MADV_HUGEPAGE);
            libc::madvise(payload_ptr as *mut libc::c_void, payload_arena_size, libc::MADV_WILLNEED);

            // Sequential for directory because expansion doubles it linearly
            libc::madvise(directory_ptr as *mut libc::c_void, directory_arena_size, libc::MADV_SEQUENTIAL);
    ...
    pub fn upsert(&self, key: K, value: V) {
        ...
        loop {
            ...
            unsafe {
                // Prefetch Keys for comaprasion operation afetr simd
                ...
                if bucket.occupancy_bitmask != 0xFFFF { // if ocupancy is not full, this is a common path for a insert 
                    ...
                    // CASE 3: No Key, But bucket has space, Split (and globally expand if needed) then InsertPayload
    ...
    // Insert
        ...
        // CASE 1: No Key Check, If there is space just insert the key into the bucket, UNSAFE CAN CAUSE DUPLICATED


        // CASE 2: If there is no space, Split&|Expand, then insert
...
if std::intrinsics::likely(bucket.occupancy < 0xFFFF) {
    // This path stays "hot" in the instruction cache
    bucket.insert_inline(key, value, hash);
} else {
    // This path is moved to the "cold" section of the binary
    self.split_and_reinsert(key, value, hash);
}
